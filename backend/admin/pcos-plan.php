...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($action === 'save_pcos_pricing') {
        ...
        $currency = htmlspecialchars(trim($_POST['pcos_currency'] ?? 'NGN'), ENT_QUOTES);
        ...
        <div class="flex items-start justify-between mb-6">
            ...
            <?php if (!empty($pcosPlan)): ?>
                <div class="bg-[#F2F4F1] border border-[#EAEAE5] rounded-xl px-5 py-4 text-right">
                    ...
                    <div class="text-xs text-[#A4B4A6]"><?php echo htmlspecialchars($pcosPlan['currency'] ?? 'NGN'); ?></div>
                </div>
            <?php endif; ?>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="pcos_currency"
                        class="w-full px-4 py-3 border border-[#EAEAE5] rounded-xl focus:outline-none focus:border-[#2C3E35] bg-white text-[#2C3E35]">
                        <?php foreach (['NGN' => '₦ Nigerian Naira (NGN)', 'USD' => '$ US Dollar (USD)', 'GBP' => '£ British Pound (GBP)'] as $val => $label): ?>
                            <option value="<?php echo $val; ?>" <?php echo ($pcosPlan['currency'] ?? 'NGN') === $val ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($label); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
