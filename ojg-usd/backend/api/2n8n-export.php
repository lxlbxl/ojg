...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$type || !$id || !$data || !is_array($data)) {
            throw new Exception('Missing required fields: type, id, data (object)');
        }

        // Allowed tables for update
        $allowedTables = ['assessments', 'users', 'sales'];
        if (!in_array($type, $allowedTables)) {
            throw new Exception("Update not allowed for type: $type");
        }

        // Perform Update
        // Database::update($table, $data, $where, $whereParams)
        // Note: We use the existing Database::update wrapper if available, or direct query

        // $db->update is available based on previous checks
        $rowCount = $db->update($type, $data, "id = :id", [':id' => $id]);

        $response = [
            'status' => 'success',
            'message' => "Updated $rowCount record(s) in $type",
            'updated_id' => $id,
            'changes' => $data
        ];
